...
SELECT cod_cliente, SUM(saldo) AS total_depositado, COUNT(*) AS num_cuentas
FROM Cuenta
...
SELECT cl.nombre, cl.apellidos, SUM(cu.saldo) AS total_depositado, COUNT(*) AS num_cuentas
FROM Cliente cl
JOIN Cuenta cu ON cl.cod_cliente = cu.cod_cliente
GROUP BY cl.cod_cliente, cl.nombre, cl.apellidos;
...
SELECT s.direccion, COUNT(cu.cod_cuenta) AS num_cuentas, SUM(cu.saldo) AS total_fondos
FROM Sucursal s
...
-- 9. Mostrar los tipos de movimientos de las cuentas bancarias, sus descripciones y el volumen total de dinero que se manejado en cada tipo de movimiento.
SELECT tm.cod_tipo_mov, tm.descripcion, SUM(m.importe) AS volumen_total
...
-- 11. Calcular la cantidad total de dinero que emite la entidad bancaria clasificada según los tipos de movimientos de salida.
SELECT tm.descripcion, SUM(m.importe) AS total_emitido
FROM Tipo_movimiento tm
JOIN Movimiento m ON tm.cod_tipo_mov = m.cod_tipo_mov
WHERE tm.salida = 1 -- 1 es TRUE en MySQL
GROUP BY tm.cod_tipo_mov, tm.descripcion;

-- 12. Calcular la cantidad total de dinero que ingresa cada cuenta bancaria clasificada según los tipos de movimientos de entrada.
SELECT m.cod_cuenta, tm.descripcion, SUM(m.importe) AS total_ingresado
FROM Movimiento m
JOIN Tipo_movimiento tm ON m.cod_tipo_mov = tm.cod_tipo_mov
WHERE tm.salida = 0 -- 0 es FALSE (Entradas)
GROUP BY m.cod_cuenta, tm.cod_tipo_mov, tm.descripcion;

-- 13. Calcular la cantidad total de dinero que sale de la entidad bancaria mediante cualquier movimiento de “salida”.
SELECT SUM(m.importe) AS total_global_salidas
FROM Movimiento m
JOIN Tipo_movimiento tm ON m.cod_tipo_mov = tm.cod_tipo_mov
WHERE tm.salida = 1;
...
SELECT cl.apellidos, cl.nombre, cu.cod_cuenta, tm.descripcion, SUM(m.importe) AS total_acumulado
FROM Cliente cl
JOIN Cuenta cu ON cl.cod_cliente = cu.cod_cliente
...
SELECT COUNT(*) AS cuentas_sin_movimientos
FROM Cuenta c
...
SELECT s.cod_sucursal, s.direccion, s.capital_anio_anterior, SUM(c.saldo) AS suma_saldos_actual
FROM Sucursal s
...
SELECT c.cod_cuenta, c.saldo, tm.descripcion, SUM(m.importe) AS suma_movimiento
FROM Cuenta c
...
SELECT cl.cod_cliente, cl.nombre, cl.apellidos, s.cod_sucursal, SUM(cu.saldo) AS saldo_cliente_sucursal, s.capital_anio_anterior
FROM Cliente cl
JOIN Cuenta cu ON cl.cod_cliente = cu.cod_cliente
